...
import {
  ...
  putSessionInCookie,
} from "src/server/session-cookie.server";
...
export const action = async ({ request }: ActionFunctionArgs) => {
  ...
  const name = String(body.get("email-or-name"));
  ...
  if (!user) return { error: "Ați greșit numele și parola!" };

  const sessionIdQuery = await database.query(
    "INSERT INTO user_session VALUES (DEFAULT, $1) RETURNING id;",
    [user.id]
  );

  const sessionId = sessionIdQuery.rows[0];
